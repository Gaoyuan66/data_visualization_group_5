...
```{r}
net_migration <- gov_data_final %>% ggplot(aes(x=year,y=net_migration,color=country_name) , format(y, scientific = FALSE))+
  geom_line(size=0.5,show.legend = FALSE)+
  scale_colour_manual(values = c("India" = "#D0BC52","China"="#FF6C67","Spain" = "#36D6D9","South Africa"= "#3CD175","United Kingdom"= "#8CBDFF","United States"= "#FF90F1"))+
  labs(title = "Western countries have positive net migration over the years", 
       subtitle = "Net migration in the country measured by million people",
       x="Year",
       y="Net migration in million",
       size=13)+
  geom_text(
    data = data.frame(x = rep(2017.5,6), y = c(-2.6,-1.8,0.2,0.8,1.2,4.8), 
                      label = c("India","China","Spain","South Africa","United Kingdom","United States")),
    aes(x = x, y = y, label = label, fontface = 4),
    colour=c("#D0BC52","#FF6C67","#36D6D9","#3CD175","#8CBDFF","#FF90F1"),
    family="Arial",
  ...
  geom_text(
    aes(x = 2019, y = -0.2, label = "World", fontface = 4),
    colour="gray70",
    family="Arial",
    hjust = 0,
    lineheight = .3,
  ...
  xlim(2002,2020)+
  annotate("rect",xmin=-Inf,xmax=Inf,ymin=0, ymax=Inf, fill="skyblue2",alpha=0.1)+
  annotate("rect",xmin=-Inf,xmax=Inf,ymin=-Inf, ymax=0, fill="gray90",alpha=0.5)
...
gov_averages %>% 
  ggplot(aes(y = avg_across_migration, 
             x = fct_reorder(country_name, -avg_across_migration), fill=country_name)) +
  geom_col()+
  scale_colour_manual(values = c("India" = "#D0BC52","China"="#FF6C67","Spain" = "#36D6D9","South Africa"= "#3CD175","United Kingdom"= "#8CBDFF","United States"= "#FF90F1"))+
labs(title = "Political stability is higher for countries with net positive migration", 
     subtitle = "Average 10 year estimate political stability and absence of violence/terrorism from 2002-2012",
       x="Country",
       y="Average Political stability estimate",
       size=13)+
        theme(legend.position = "none",
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_line(colour = "grey"),
        ...
        axis.line = element_line(colour = "grey"),
        plot.background=element_blank())+
  annotate("rect",xmin=-Inf,xmax="South Africa",ymin=-Inf, ymax=Inf, fill="skyblue2",alpha=0.1)+
  annotate("rect",xmin="South Africa",xmax=Inf,ymin=-Inf, ymax=Inf, fill="gray90",alpha=0.5)+
    geom_hline(yintercept=0, linetype="dashed", color="gray60", size=1)
